<?php
    ...
    if ($_POST["Actividad"] =='lst_cotizaciones')
    {
        $cod_user=$_SESSION['codusu'];
        $query ="SELECT ROW_NUMBER() OVER(ORDER BY DATE_FORMAT(cot_fecha, '%Y-%m-%d') ASC) AS 'fila',  year(cot_fecha), cli_nombre FROM t_cotizacion b
