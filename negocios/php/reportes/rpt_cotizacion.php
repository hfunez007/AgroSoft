...
$registros = 25;
$ip = 36;
$nb_page = ceil($total_registros / $registros);

if ($total_registros < $ip) {
	$totalotros = $ip;
} else {
	$totalotros = $total_registros - $ip;
}


$num_page = 1;
$limit_inf = 0;
$limit_sup = $totalotros;
...
while ($num_page <= $nb_page) {
	...
	$pdf->SetLineWidth(0.1);
	$pdf->Rect(5, 75, 200, 138, "D");

	// cadre titre des colonnes
	$pdf->Line(5, 85, 205, 85);

	// les traits verticaux colonnes
	$pdf->Line(20, 75, 20, 213);
	$pdf->Line(50, 75, 50, 213);
	$pdf->Line(165, 75, 165, 213);
	$pdf->Line(183, 75, 183, 213);
	...
	while ($row1 =  mysqli_fetch_array($res)) {
		// cantidad
		$pdf->SetXY(5, $y + 10);
		// $pdf->Cell(15, $ancho, utf8_decode($row1[3]), '', 0, 'C');
		$pdf->Cell(15, $ancho, utf8_decode($totalotros), '', 0, 'C');

		// UNIDAD
		$pdf->SetXY(20, $y + 10);
		$pdf->Cell(30, $ancho, utf8_decode($row1[10]), '', 0, 'C');

		// DESCRIPCION		
		$pdf->SetXY(50, $y + 10);
		$pdf->Cell(120, $ancho, utf8_decode($row1[2]), '', 0, 'L');

		// PRECIO
		$pdf->SetXY(165, $y + 10);
		$pdf->Cell(18, $ancho, utf8_decode($row1[9]), '', 0, 'L');

		// total
		$pdf->SetXY(183, $y + 10);
		$pdf->Cell(17, $ancho, utf8_decode($row1[5]), '', 0, 'L');

		// $pdf->Line(5, $y + 13, 205, $y + 13);
